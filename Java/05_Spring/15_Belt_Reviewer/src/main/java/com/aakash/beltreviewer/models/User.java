...
public class User {
	...
	public boolean hasRing(Ring ring){
		for(Ring _ring:this.rings){
			if(_ring.getName().equals(ring.getName()) || _ring.getId() == ring.getId()){return true;}
		}
		return false;
	}
	...
	public boolean isInGuild(Guild guild){
		for(Guild _guild:this.guilds){
			if(_guild.getName().equals(guild.getName()) || _guild.getId() == guild.getId()){return true;}
		}
		return false;
	}
